...
async function testConnection() {
  try {
    // Use a simple getDoc on the test path we just added to rules
    ...
    if (error instanceof Error) {
      if (error.message.includes('the client is offline') || error.message.includes('Could not reach Cloud Firestore backend')) {
        console.error("Firestore connection timeout or offline. Please check your Firebase configuration and internet connection.");
      } else if (error.message.includes('Missing or insufficient permissions')) {
        console.warn("Firestore connection test failed due to permissions (expected if not yet deployed).");
      } else {
...
export { signInWithPopup, onAuthStateChanged, collection, onSnapshot, query, where, orderBy, limit, setDoc, addDoc, updateDoc, deleteDoc, getDocs, getDoc };
